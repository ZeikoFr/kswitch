...
If you have set up multiple DigitalOcean accounts with `doctl`, then `Kswitch` will discover the DOKS cluster from all accounts.

If you are already using `doctl`, please see the [section for existing `doctl` users below](#existing-doctl-users), otherwise you need to [first setup `doctl`](#how-to-setup-doctl-for-kswitch).
...
`Kswitch` aims to "just work" for existing `doctl` users, hence there is no setup to be done. Just fire-up `kswitch` and see the discovered DOKS clusters.
...
# How to setup `doctl` for `kswitch`

Kswitch re-uses the existing `doctl` configuration including its means of authentication.
...
`Kswitch` currently only reads the `doctl` created `config.yaml` from [its default location](https://github.com/digitalocean/doctl?tab=readme-ov-file#configuring-default-values). 
...
Using `doctl auth init` will write the credentials to the `config.yaml` for `kswitch` to discover it.