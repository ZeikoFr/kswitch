...
of `Shoot` clusters and `Managed Seeds`.
External `Seeds` (not managed) are currently not supported.

## Prerequisites

**Requirements on your local machine**

- The `gardenlogin` credentials plugin must be installed on the client side.
  Please check the [installation documentation](https://github.com/gardener/gardenlogin#installation).

For your convenience, kswitch automatically generates and maintains two credential-plugin config files
based on the configured Gardener stores — you do not need to create them manually.

### gardenlogin config (`~/.garden/gardenlogin.yaml`)

Required by the `gardenlogin` kubectl auth plugin to fetch short-lived client certificates for Shoot clusters.

```yaml
# ~/.garden/gardenlogin.yaml — auto-generated by kswitch
gardens:
  - identity: sap-landscape-dev  # matches the cluster-identity ConfigMap in kube-system of the garden cluster
    kubeconfig: /path/to/garden/kubeconfig
  - identity: my-other-landscape
    kubeconfig: /path/to/other/kubeconfig
```

### gardenctl-v2 config (`~/.garden/gardenctl-v2.yaml`)

Required by [gardenctl-v2](https://github.com/gardener/gardenctl-v2) to target garden clusters.
Kswitch maintains this file automatically so that `gardenctl` works side-by-side without separate configuration.

```yaml
# ~/.garden/gardenctl-v2.yaml — auto-generated by kswitch
gardens:
  - identity: sap-landscape-dev
    kubeconfig: /path/to/garden/kubeconfig
  - identity: my-other-landscape
    kubeconfig: /path/to/other/kubeconfig
```
...
- Gardener >= `v1.43.0` — the Gardenlet syncs the Shoot CA secret (`<shoot-name>.ca-cluster`) to the
  garden cluster namespace, which kswitch uses to construct Shoot kubeconfigs directly without any
  additional controller. Older installations are not supported.
