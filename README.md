...
Available Commands:
  ...
  version              show kswitch version info

Flags:
      --config-path string         path on the local filesystem to the configuration file. (default "/Users/tommyolsen/.kube/kswitch-config.yaml")
      --debug                      show debug logs
  -h, --help                       help for kswitch
      ...
      --state-directory string     path to the local directory used for storing internal state. (default "/Users/tommyolsen/.kube/kswitch-state")
  ...
  -v, --version                    version for kswitch

Use "kswitch [command] --help" for more information about a command.
```

Just type `kswitch` to search over the context names defined in the default Kubeconfig file `~/.kube/config`
...
Change the current namespace using `kswitch ns`
...
Similar to the command histories of a shell, `kswitch` keeps a history of used contexts and namespaces.
...
In addition, use 
- `kswitch .` to change to the last used context and namespace (handy for new terminals)
- `kswitch -` to change to the previous history entry

## List and search for contexts

You can list all your indexed contexts by issuing the following command: `kswitch list-contexts`. 
And if you want to search for only parts of those contexts, you can use wildcard search:

```sh
kswitch list-contexts "*-dev-?"
```
...
```sh
kswitch exec "*-dev-?" -- kubectl get ns
```

You can also wrap the command(s) into a script and execute it via `kswitch exec`:

```sh
kswitch exec "*-dev-?" -- bash script.sh
```

For inline shell script execution, you need to specify the default shell in the `SwitchConfig` file:

```yaml
# ~/.kube/kswitch-config.yaml
kind: SwitchConfig
execShell: "/bin/bash"
```
...
kswitch exec "*-dev-?" -- 'for i in 1 2 3; do sleep 1; echo "hi $i"; done'
```
...
```
$ kswitch alias mediathekview=gke_mediathekviewmobile-real_europe-west1-c_mediathekviewmobile
```

It is also possible to use `kswitch alias <alias>=.` to create an alias for the current context.
...
```
$ kswitch alias rm mediathekview
```
...
Likewise, use `kswitch <NEW_NAME>=<NAME>` to create an alias.
An alias **does not modify** or rename the context in the kubeconfig file. 
It is just a local configuration that can be removed again via `kswitch alias rm <NAME>`.
